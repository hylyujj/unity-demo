...
public class MathUtil
{
    ...
    /// <summary>
    ///     判断两个RectTransform是否相交
    /// </summary>
    /// <param name="rect1"></param>
    /// <param name="rect2"></param>
    /// <returns></returns>
    public static bool IsRectTransformsIntersecting(RectTransform rect1, RectTransform rect2)
    {
        // 获取两个RectTransform的世界空间坐标
        var corners1 = new Vector3[4];
        var corners2 = new Vector3[4];
        rect1.GetWorldCorners(corners1);
        rect2.GetWorldCorners(corners2);


        var rectWorld1 = new Rect(corners1[0].x, corners1[0].y,
            corners1[2].x - corners1[0].x, corners1[2].y - corners1[0].y);
        var rectWorld2 = new Rect(corners2[0].x, corners2[0].y,
            corners2[2].x - corners2[0].x, corners2[2].y - corners2[0].y);
        return rectWorld1.Overlaps(rectWorld2);
    }

    public static float RectTranformsIntersectionProportion(RectTransform rect1, RectTransform rect2)
    {
        // 获取两个RectTransform的世界空间坐标
        var corners1 = new Vector3[4];
        var corners2 = new Vector3[4];
        ...
        var rectWorld2 = new Rect(corners2[0].x, corners2[0].y,
            corners2[2].x - corners2[0].x, corners2[2].y - corners2[0].y);
        if (!rectWorld1.Overlaps(rectWorld2))
            return 0;
        var xMin = MathF.Max(rectWorld1.xMin, rectWorld2.xMin);
        var xMax = MathF.Min(rectWorld1.xMax, rectWorld2.xMax);
        var yMin = Mathf.Max(rectWorld1.yMin, rectWorld2.yMin);
        var yMax = Mathf.Min(rectWorld1.yMax, rectWorld2.yMax);
        var intersection = (xMax - xMin) * (yMax - yMin);
        return intersection / (rectWorld1.height * rectWorld1.width);
    }

    public static float RectTranformsIntersectionProportion(RectTransform rect1, Rect rectWorld2)
    {
        // RectTransform 和 Rect比较
        var corners1 = new Vector3[4];
        rect1.GetWorldCorners(corners1);
        ...
        //Debug.Log($"ratio: {intersection / (rectWorld1.height * rectWorld1.width)}\n" +
        //    $"{rectWorld1.xMin}, {rectWorld1.xMax}, {rectWorld1.yMin}, , {rectWorld1.yMax}\n" +
        //    $"{rectWorld2.xMin}, {rectWorld2.xMax}, {rectWorld2.yMin}, , {rectWorld2.yMax}");
        return intersection / (rectWorld1.height * rectWorld1.width);
    }
    ...
    {
        var type = typeof(T);
        var colliders = Physics.OverlapBox(center, halfExtents, rotation, layerMask, QueryTriggerInteraction.Collide);
