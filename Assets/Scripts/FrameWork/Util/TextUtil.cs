...
public class TextUtil
{
    ...
    public static string[] SplitStr(string str, int type = 1)
    {
        ...
        if (type == 6)
            return newStr.Split('|');
        if (type == 7) return newStr.Split('_');
    ...
    {
        //得到拆分后的字符串数组
        var strs = SplitStr(str, type);
        if (strs.Length == 0)
            return new int[0];
        //把字符串数组 转换成 int数组 
        return Array.ConvertAll(strs, str => { return int.Parse(str); });
    }
